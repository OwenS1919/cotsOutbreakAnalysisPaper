function probVec = compFuncWeibWeib(tVec, paramsStruct)
...
% paramsStruct - a structure holding the relevant parameters a.k.a. a, the
    ...
    % initial Weibull loss in competence, b2, the lambda parameter for the
    % second Weibull loss in competence after Tcp, v2, the nu parameter for
    % the second Weibull loss in competence after Tcp, Tcp, the point at
    % which the loss in competence changes from the first to second
    % Weibull, tc, the earliest time at which competency is gained
...
laterInt1 = @(tau, t) paramsStruct.a * exp(-paramsStruct.a * (tau ...
    ...
    - paramsStruct.Tcp)).^paramsStruct.v2);
